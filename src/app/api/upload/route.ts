...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('  image :', image ? `${image.name} (${image.size} bytes, ${image.type})` : 'なし');
    console.log('='.repeat(60));
    ...
    if (!mid || !bodai || !spot) {
      return NextResponse.json(
        { success: false, error: '必須パラメータが不足しています (mid, bodai, spot)' },
        { status: 400 }
      );
    }

    // 開発用：実際にはここで PHP サーバーに転送するか、
    // ストレージに保存して Pub/Sub にパブリッシュする
